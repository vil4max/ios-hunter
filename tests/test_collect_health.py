...
from database.collect_slots import mark_slot_completed, load_collect_slots
...
def test_overdue_only_after_grace_and_not_completed(tmp_path):
    ...
    assert overdue_slots(path, datetime(2026, 9, 9, 9, 14, tzinfo=KYIV)) == []
    assert overdue_slots(path, datetime(2026, 9, 9, 9, 15, tzinfo=KYIV)) == [9]
    mark_slot_completed(path, '2026-09-09', 12)
    assert overdue_slots(path, datetime(2026, 9, 9, 15, 30, tzinfo=KYIV)) == [9, 15]
...
def test_slow_collect_marks_start_slot_not_finish_slot(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv('DAILY_EMAIL_DAYS_PATH', str(tmp_path / 'email.json'))
    monkeypatch.setenv('COLLECT_STARTED_AT', '2026-09-09T08:58:00Z')
    assert mark_collect_slot.main() == 0
    assert load_collect_slots(path)['days']['2026-09-09']['slots'] == [9]
