...
def test_next_scheduled_collect_same_day() -> None:
    ...
    assert next_scheduled_collect(now) == datetime(2026, 7, 28, 10, 0, tzinfo=_KYIV)
    assert format_next_check_line(now) == "⏭ Следующая проверка в 10:00"


def test_next_scheduled_collect_last_slot() -> None:
    now = datetime(2026, 7, 28, 17, 10, tzinfo=_KYIV)
